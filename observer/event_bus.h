...
public:
	...
	virtual bool compare(const SignalRegistration *r) const {
		const TypedSignalRegistration *tr =
				static_cast<const TypedSignalRegistration*>(r);
...
public:
	...
	void addListener(ObserverT *o, EventT *e, SubjectT *s, bool isOneShot) {
		...
		//new (Pool.allocate()) TypedSignalRegistration<ObserverT, EventT,SubjectT>(o, isOneShot);
				new TypedSignalRegistration<ObserverT, EventT, SubjectT>(o,
						isOneShot);
		intptr_t eventKey = tsr->getEventKey();
		auto p = BusMap.insert(
				etl::make_pair(eventKey, MAP_SUB_TO_LISTENER())
		);
		auto p2 = (*p.first).second.insert(
				etl::make_pair(((uint32_t) s), SIGNAL_LIST())
		);
