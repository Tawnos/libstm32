...
using cmdc0de::LogManager;
using cmdc0de::Logger;
using cmdc0de::LOG_LEVEL;
...
static Logger *Loggers[cmdc0de::NUMBER_OF_LOGGERS] = {0};

void LogManager::addLogger(uint32_t id, Logger *l) {
	if(id<cmdc0de::NUMBER_OF_LOGGERS) return;
	Loggers[id] = l;
}

Logger *LogManager::getLogger(const uint32_t &id) {
	if(id>=cmdc0de::NUMBER_OF_LOGGERS) return 0;
	return Loggers[id];
}
...
cmdc0de::STM32Logger::STM32Logger(LOG_LEVEL l) : Logger(l) {
